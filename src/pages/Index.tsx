...
const Index = () => {
  ...
  /* Department filter for timeline in "All Projects" */
  ...
  /* Helper: check if viewing a specific project (not a virtual view) */
  const isSpecificProject = !['all', 'my', 'dept'].includes(selectedProjectId);
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-14 flex items-center justify-between px-4 md:px-6 border-b bg-card">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              ...
              {isSpecificProject && selectedProject && <p className="text-xs text-muted-foreground hidden md:block">{selectedProject.description}</p>}
          ...
          <div className="flex items-center gap-1.5 md:gap-2">
            {/* Kanban toggle (mobile only, specific project) */}
            {isMobile && isSpecificProject && (
              <button onClick={() => setKanbanOpen(v => !v)} className="p-1.5 rounded-md text-muted-foreground hover:bg-muted transition-colors" title={kanbanOpen ? 'Hide Kanban' : 'Show Kanban'}>
                {kanbanOpen ? <X className="w-4 h-4" /> : <LayoutDashboard className="w-4 h-4" />}
              </button>
            )}
            {/* Edit project & add task buttons (specific project only) */}
            {selectedProject && isSpecificProject && (
              <>
                <button onClick={() => setShowEditProject(true)} className="p-1.5 rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title="Edit project">
                  <Settings className="w-4 h-4" />
                </button>
                <button onClick={() => setShowAddTask(true)} className="flex items-center gap-1.5 px-2 md:px-3 py-1.5 rounded-md text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
        ...
        <div className="flex-1 overflow-auto">
          ...
          {isSpecificProject && tasks.length > 0 && (
          ...
          {/* ── All Projects / My Projects overview ── */}
          {(selectedProjectId === 'all' || selectedProjectId === 'my') && (
            <div className="px-4 md:px-6 py-6 space-y-5">
              {loading ? (
                ...
                <>
                  {/* Pie chart + per-project breakdown */}
                  <div className="bg-card rounded-xl border p-4 md:p-5">
                    <h2 className="text-sm font-semibold text-card-foreground mb-4 flex items-center gap-2">
                  ...
                  {/* Department progress (All Projects only) */}
                  ...
                  {/* Timeline with optional department filter */}
                  <div className="bg-card rounded-xl border p-4 md:p-5">
            ...
            <div className="px-4 md:px-6 py-6 space-y-5">
              {loading ? (
                ...
                <>
                  ...
                  {/* Per-project breakdown for the department */}
                  <div className="bg-card rounded-xl border p-4 md:p-5">
                    <h2 className="text-sm font-semibold text-card-foreground mb-4 flex items-center gap-2">
                      <PieIcon className="w-4 h-4 text-primary" />
      ...
      {/* Global floating AI assistant — visible across every view */}
      <ChatBot />
    </div>
  );
};
